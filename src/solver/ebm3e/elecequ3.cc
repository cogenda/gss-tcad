...
void ElZone::F3_ddm_inner(int i,PetscScalar *x,PetscScalar *f, ODE_Formula &ODE_F, vector<int> & zofs)
{
...
void ElZone::F3_ddm_neumannbc(int i,PetscScalar *x,PetscScalar *f, ODE_Formula &ODE_F, vector<int> & zofs,DABC &bc)
{
  const VoronoiCell* pcell = &pzone->davcell[i];
...
void ElZone::F3_ddm_ombc_interface(int i,PetscScalar *x,PetscScalar *f, ODE_Formula &ODE_F, vector<int> & zofs, DABC &bc,
                                   SMCZone *pz, int n)
{
  f[zofs[zone_index]+2*i+0] = x[zofs[zone_index]+2*i+0] - x[zofs[pz->zone_index]+6*n+0];
  f[zofs[zone_index]+2*i+1] = x[zofs[zone_index]+2*i+1] - x[zofs[pz->zone_index]+6*n+3];
}


void ElZone::F3_ddm_stkbc_interface(int i,PetscScalar *x,PetscScalar *f, ODE_Formula &ODE_F, vector<int> & zofs, DABC &bc,
                                    SMCZone *pz, int n)
{
  f[zofs[zone_index]+2*i+0] = x[zofs[zone_index]+2*i+0] - x[zofs[pz->zone_index]+6*n+0];
  f[zofs[zone_index]+2*i+1] = x[zofs[zone_index]+2*i+1] - x[zofs[pz->zone_index]+6*n+3];
...
void ElZone::F3_ddm_gatebc_interface(int i,PetscScalar *x,PetscScalar *f, ODE_Formula &ODE_F, vector<int> & zofs, DABC &bc,
                                     ISZone *pz, int n)
{
...
void ElZone::F3_ddm_elec_insulator_interface(int i,PetscScalar *x,PetscScalar *f, ODE_Formula &ODE_F, vector<int> & zofs,DABC &bc,
                                             ISZone *pz, int n)
{
  const VoronoiCell* pcell = &pzone->davcell[i];
  PetscScalar Vi = x[zofs[zone_index]+2*i+0];
...
void ElZone::F3_ddm_chargebc_interface(int i,PetscScalar *x,PetscScalar *f, ODE_Formula &ODE_F, vector<int> & zofs, DABC &bc,
                                     ISZone *pz, int n)
{
  const VoronoiCell* pcell = &pzone->davcell[i];
  PetscScalar Ti = x[zofs[zone_index]+2*i+1];
...
void ElZone::J3_ddm_inner(int i,PetscScalar *x,Mat *jac, ODE_Formula &ODE_F, vector<int> & zofs)
{
  const VoronoiCell* pcell = &pzone->davcell[i];
  PetscScalar Ti = x[zofs[zone_index]+2*i+1];
...
void ElZone::J3_ddm_neumannbc(int i,PetscScalar *x,Mat *jac,ODE_Formula &ODE_F, vector<int> &zofs,DABC &bc)
{
  const VoronoiCell* pcell = &pzone->davcell[i];
  NeumannBC *pbc = dynamic_cast <NeumannBC * >(bc.Get_pointer(pcell->bc_index-1));
  PetscScalar Ti = x[zofs[zone_index]+2*i+1];
...
void ElZone::J3_ddm_ombc_interface(int i,PetscScalar *x,Mat *jac, ODE_Formula &ODE_F, vector<int> & zofs, DABC &bc,
                                   SMCZone *pz, int n)
{
  MatSetValue(*jac,zofs[zone_index]+2*i+0,zofs[zone_index]+2*i+0,1.0,INSERT_VALUES);
  MatSetValue(*jac,zofs[zone_index]+2*i+0,zofs[pz->zone_index]+6*n+0,-1.0,INSERT_VALUES);

  MatSetValue(*jac,zofs[zone_index]+2*i+1,zofs[zone_index]+2*i+1,1.0,INSERT_VALUES);
  MatSetValue(*jac,zofs[zone_index]+2*i+1,zofs[pz->zone_index]+6*n+3,-1.0,INSERT_VALUES);
}


void ElZone::J3_ddm_stkbc_interface(int i,PetscScalar *x,Mat *jac, ODE_Formula &ODE_F, vector<int> & zofs, DABC &bc,
                                   SMCZone *pz, int n)
{
  MatSetValue(*jac,zofs[zone_index]+2*i+0,zofs[zone_index]+2*i+0,1.0,INSERT_VALUES);
  MatSetValue(*jac,zofs[zone_index]+2*i+0,zofs[pz->zone_index]+6*n+0,-1.0,INSERT_VALUES);
...
void ElZone::J3_ddm_gatebc_interface(int i,PetscScalar *x,Mat *jac, ODE_Formula &ODE_F, vector<int> & zofs, DABC &bc,
                                     ISZone *pz, int n)
{
  MatSetValue(*jac,zofs[zone_index]+2*i+0,zofs[zone_index]+2*i+0,1.0,INSERT_VALUES);
...
void ElZone::J3_ddm_elec_insulator_interface(int i,PetscScalar *x,Mat *jac, ODE_Formula &ODE_F, vector<int> & zofs, DABC &bc,
                                   ISZone *pz, int n)
{
  const VoronoiCell* pcell = &pzone->davcell[i];
  PetscScalar Ti = x[zofs[zone_index]+2*i+1];
...
void ElZone::J3_ddm_chargebc_interface(int i,PetscScalar *x,Mat *jac, ODE_Formula &ODE_F, vector<int> & zofs, DABC &bc,
                                   ISZone *pz, int n)
{
  const VoronoiCell* pcell = &pzone->davcell[i];
  PetscScalar Ti = x[zofs[zone_index]+2*i+1];

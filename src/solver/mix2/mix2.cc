...
/* ----------------------------------------------------------------------------
 * error_norm_pn_Mix2:  This function compute X and RHS error norms. 
 */
void DDM_Mix_Solver_L2E:: error_norm_pn_Mix2(PetscScalar *x,PetscScalar *f)
{
  // do clear
  potential_norm   = 0;
  electron_norm    = 0;
 ...
 */
PetscErrorCode LineSearch_ConvergenceTest_Mix2(SNES snes,PetscInt it,PetscReal xnorm, PetscReal pnorm, PetscReal fnorm,
    SNESConvergedReason *reason,void *dummy)
{
  DDM_Mix_Solver_L2E *ps = (DDM_Mix_Solver_L2E*)dummy;
 ...
 */
PetscErrorCode TrustRegion_ConvergenceTest_Mix2(SNES snes,PetscInt it,PetscReal xnorm, PetscReal pnorm, PetscReal fnorm,
    SNESConvergedReason *reason,void *dummy)
{
 ...
 */
void DDM_Mix_Solver_L2E::form_function_pn_Mix2(PetscScalar *x,PetscScalar *f)
{
    ...
    if(zonedata[z]->material_type==Semiconductor)
    {
      ...
      for(int i=0;i<pzonedata->electrode.size();i++)
      {
        if(bc[pzonedata->electrode[i]].BCType==OhmicContact)
          pzonedata->F2E_mix_om_electrode_current(i,x,f,ODE_formula,zofs,bc,DeviceDepth);
        if(bc[pzonedata->electrode[i]].BCType==SchottkyContact)
          pzonedata->F2E_mix_stk_electrode_current(i,x,f,ODE_formula,zofs,bc,DeviceDepth);
        if(bc[pzonedata->electrode[i]].BCType==InsulatorContact)
          pzonedata->F2E_mix_ins_electrode_current(i,x,f,ODE_formula,zofs,bc,DeviceDepth);
      }
      ...
      for(int i=0;i<zone[z].davcell.size();i++)
      {
        ...
        else if(pcell->bc_index&&bc[pcell->bc_index-1].BCType==OhmicContact)
        {
          ...
          if(pbc->pinterface)//the ohmic bc is an electrode region
          {
            ...
            pzonedata->F2E_mix_ddm_ombc_interface(i,x,f,ODE_formula,zofs,bc,pz,n_node);
          }
          else //the ohmic bc is a segment
            pzonedata->F2E_mix_ddm_ombc_segment(i,x,f,ODE_formula,zofs,bc);
        }
        else if(pcell->bc_index&&bc[pcell->bc_index-1].BCType==SchottkyContact)
        {
          ...
          if(pbc->pinterface)//the Schottky bc is an electrode region
          {
            ...
            int n_node = pbc->pinterface->Find_neighbor_node_index(z,i);
            ElZone * pz = dynamic_cast<ElZone *>(zonedata[n_zone]);
            pzonedata->F2E_mix_ddm_stkbc_interface(i,x,f,ODE_formula,zofs,bc,pz,n_node);
          }
          else //the Schottky bc is a segment
            pzonedata->F2E_mix_ddm_stkbc_segment(i,x,f,ODE_formula,zofs,bc);
        }
        else if(pcell->bc_index&&bc[pcell->bc_index-1].BCType==InsulatorContact)
          pzonedata->F2E_mix_ddm_insulator_gate(i,x,f,ODE_formula,zofs,bc);
        // the routine for interface BC do not change, share the DDML2E routine.
    ...
    if(zonedata[z]->material_type==Insulator)
    {
      ...
      for(int i=0;i<zone[z].davcell.size();i++)
      {
        ...
        else if(pcell->bc_index&&bc[pcell->bc_index-1].BCType==GateContact)
        {
          ...
          if(pbc->pinterface)//the gate bc is an electrode region
          {
            ...
            int n_node = pbc->pinterface->Find_neighbor_node_index(z,i);
            ElZone * pz = dynamic_cast<ElZone *>(zonedata[n_zone]);
            pzonedata->F2_mix_ddm_gatebc_interface(i,x,f,ODE_formula,zofs,bc,pz,n_node);
          }
          else //the gate bc is a segment
            pzonedata->F2_mix_ddm_gatebc_segment(i,x,f,ODE_formula,zofs,bc);
        }
      ...
      for(int i=0;i<pzonedata->electrode.size();i++)
      {
        ...
        if(bc[pzonedata->electrode[i]].BCType==GateContact)
          pzonedata->F2_mix_gate_electrode_current(i,x,f,ODE_formula,zofs,bc,DeviceDepth);
 ...
 */
void DDM_Mix_Solver_L2E::form_jacobian_pn_Mix2(PetscScalar *x, Mat *jac, Mat *jtmp)
{
      ...
      for(int i=0;i<zone[z].davcell.size();i++)
      {
        ...
        else if(pcell->bc_index&&bc[pcell->bc_index-1].BCType==OhmicContact)
        {
          ...
          if(pbc->pinterface)//the ohmic bc is an electrode region
          {
            ...
            int n_node = pbc->pinterface->Find_neighbor_node_index(z,i);
            ElZone * pz = dynamic_cast<ElZone *>(zonedata[n_zone]);
            pzonedata->J2E_mix_ddm_ombc_interface(i,x,jac,jtmp,ODE_formula,zofs,bc,pz,n_node);
          }
          else //the ohmic bc is a segment
            pzonedata->J2E_mix_ddm_ombc_segment(i,x,jac,&JTmp,ODE_formula,zofs,bc);
        }
        else if(pcell->bc_index&&bc[pcell->bc_index-1].BCType==SchottkyContact)
        {
          SchottkyBC *pbc = dynamic_cast<SchottkyBC*>(bc.Get_pointer(pcell->bc_index-1));
          if(pbc->pinterface)//the Schottky bc is an electrode region
          {
            ...
            int n_node = pbc->pinterface->Find_neighbor_node_index(z,i);
            ElZone * pz = dynamic_cast<ElZone *>(zonedata[n_zone]);
            pzonedata->J2E_mix_ddm_stkbc_interface(i,x,jac,jtmp,ODE_formula,zofs,bc,pz,n_node);
          }
          else //the Schottky bc is a segment
            pzonedata->J2E_mix_ddm_stkbc_segment(i,x,jac,&JTmp,ODE_formula,zofs,bc);
        }
        else if(pcell->bc_index&&bc[pcell->bc_index-1].BCType==InsulatorContact)
          pzonedata->J2E_mix_ddm_insulator_gate(i,x,jac,&JTmp,ODE_formula,zofs,bc);
        // the routine for interface BC do not change, share the DDML2E routine.
        else if(pcell->bc_index&&bc[pcell->bc_index-1].BCType==InsulatorInterface)
        {
          InsulatorInterfaceBC *pbc;
      ...
      for(int i=0;i<zone[z].davcell.size();i++)
      {
        ...
        else if(pcell->bc_index&&bc[pcell->bc_index-1].BCType==GateContact)
        {
          ...
          if(pbc->pinterface)//the gate bc is an electrode region
          {
            ...
            int n_node = pbc->pinterface->Find_neighbor_node_index(z,i);
            ElZone * pz = dynamic_cast<ElZone *>(zonedata[n_zone]);
            pzonedata->J2_mix_ddm_gatebc_interface(i,x,jac,ODE_formula,zofs,bc,pz,n_node);
          }
          else //the gate bc is a segment
            pzonedata->J2_mix_ddm_gatebc_segment(i,x,jac,ODE_formula,zofs,bc);
        }
        else if(pcell->bc_index && bc[pcell->bc_index-1].BCType==IF_Electrode_Insulator)
        {
          ElectrodeInsulatorBC *pbc = dynamic_cast<ElectrodeInsulatorBC*>(bc.Get_pointer(pcell->bc_index-1));
...
/* ----------------------------------------------------------------------------
 * SNES_form_function_pn_Mix2:  wrap function for petsc nonlinear solver
 */
PetscErrorCode SNES_form_function_pn_Mix2(SNES snes, Vec x,Vec f,void *dummy)
{
  PetscScalar    *xx,*ff;
  DDM_Mix_Solver_L2E *ps = (DDM_Mix_Solver_L2E*)dummy;
  VecZeroEntries(f);
  ...
  ps->form_function_pn_Mix2(xx,ff);
  ps->error_norm_pn_Mix2(xx,ff);
...
/* ----------------------------------------------------------------------------
 * SNES_form_jacobian_pn_Mix2:  wrap function for petsc nonlinear solver
 */
PetscErrorCode SNES_form_jacobian_pn_Mix2(SNES snes, Vec x,Mat *jac,Mat *B,MatStructure *flag,void *dummy)
{
  PetscScalar    *xx;
  DDM_Mix_Solver_L2E *ps = (DDM_Mix_Solver_L2E*)dummy;
  ...
  //build matrix here
  ps->form_jacobian_pn_Mix2(xx,jac,&ps->JTmp);
...
PetscErrorCode LimitorByBankRose_Mix2(SNES snes, Vec x,Vec y,Vec w,void *checkctx, PetscTruth *changed_y,PetscTruth *changed_w)
{
  int it;
  DDM_Mix_Solver_L2E *ps = (DDM_Mix_Solver_L2E*)checkctx;
  PetscScalar    *xx;
  PetscScalar    *yy;
  PetscScalar    *ww;
  VecGetArray(x,&xx);
...
PetscErrorCode LimitorByPotential_Mix2(SNES snes, Vec x,Vec y,Vec w,void *checkctx, PetscTruth *changed_y,PetscTruth *changed_w)
{
  PetscScalar    *xx;
  PetscScalar    *yy;
  PetscScalar    *ww;
...
PetscErrorCode LimitorNonNegativeCarrier_Mix2(SNES snes, Vec x,Vec y,Vec w,void *checkctx, PetscTruth *changed_y,PetscTruth *changed_w)
{
  PetscScalar    *xx;
  PetscScalar    *yy;
  PetscScalar    *ww;
...
/* ----------------------------------------------------------------------------
 * DDM_Mix_Solver_L2E::init_solver:  This function do initial setup for nonlinear solver
 */
int DDM_Mix_Solver_L2E::init_solver(SolveDefine &sv)
{
  gss_log.string_buf()<<"DDM Level 2E Device/Circuit Mix Solver init...\n";
  gss_log.record();
  psv = &sv;
  //connect to ngspice
  ...
  // compute the scale of problem. NO extra electrode equation needs.
  ...
  SNESSetFunction(snes,r,SNES_form_function_pn_Mix2,this);
  SNESSetJacobian(snes,J,J,SNES_form_jacobian_pn_Mix2,this);
  ...
  if(sv.NS==LineSearch || sv.NS==Basic)
  {
    ...
    if(sv.Damping==DampingBankRose)
      SNESLineSearchSetPostCheck(snes,LimitorByBankRose_Mix2,this);
    else if(sv.Damping==DampingPotential)
       SNESLineSearchSetPostCheck(snes,LimitorByPotential_Mix2,this);
    else
       SNESLineSearchSetPostCheck(snes,LimitorNonNegativeCarrier_Mix2,this);
       
    SNESSetTolerances(snes,1e-12*N,1e-14,1e-9,maxit,1000);
    SNESSetConvergenceTest(snes,LineSearch_ConvergenceTest_Mix2,this);
  }
  else if(sv.NS==TrustRegion)
  {
    ...
    SNESSetConvergenceTest(snes,TrustRegion_ConvergenceTest_Mix2,this);
  ...
  gss_log.string_buf()<<"DDM Level 2E Device/Circuit Mix Solver init ok.\n";
  gss_log.record();
  return 0;
}
...
/* ----------------------------------------------------------------------------
 * DDM_Mix_Solver_L2E::do_solve:  This function receive ngspice device information.
 */
int DDM_Mix_Solver_L2E::do_solve(SolveDefine &sv)
{
...
int  DDM_Mix_Solver_L2E::DEV_LOAD()
{
  PetscScalar    *xx;
    ...
    for(int z=0;z<zone_num;z++)
    {
        ...
        for(int j=0;j<pzonedata->electrode.size();j++)
          if(bc.is_electrode_label_nocase(pzonedata->electrode[j], PINinfos[i].name))
          {
            if(bc[pzonedata->electrode[j]].BCType==OhmicContact)
              pzonedata->F2E_mix_om_electrode_Load(pzonedata->electrode[j],PINinfos[i].I,PINconds[i].pdI_pdV,
            ...
            if(bc[pzonedata->electrode[j]].BCType==SchottkyContact)
              pzonedata->F2E_mix_stk_electrode_Load(pzonedata->electrode[j],PINinfos[i].I,PINconds[i].pdI_pdV,
                                                    PINconds[i].pdI_pdw,PINconds[i].pdF_pdV,
                                                    xx,&JTmp,ODE_formula,zofs,bc,DeviceDepth);
            if(bc[pzonedata->electrode[j]].BCType==InsulatorContact)
              pzonedata->F2E_mix_ins_electrode_Load(pzonedata->electrode[j],PINinfos[i].I,PINconds[i].pdI_pdV,
                                                    PINconds[i].pdI_pdw,PINconds[i].pdF_pdV,
                                                    xx,&JTmp,ODE_formula,zofs,bc,DeviceDepth);
          }
      }
      ...
      if(zonedata[z]->material_type==Insulator)
      {
        ...
        for(int j=0;j<pzonedata->electrode.size();j++)
          if(bc.is_electrode_label_nocase(pzonedata->electrode[j], PINinfos[i].name))
          {
            pzonedata->F2_mix_gate_electrode_Load(pzonedata->electrode[j],PINinfos[i].I,PINconds[i].pdI_pdV,
                                                  PINconds[i].pdI_pdw,PINconds[i].pdF_pdV,
...
int  DDM_Mix_Solver_L2E::DEV_ACCEPT()
{
  solution_update();
  return 0;
}
...
int  DDM_Mix_Solver_L2E::DEV_CONV_TEST()
{
...
int DDM_Mix_Solver_L2E::tran_solve()
{
  PetscInt rework = 1;
...
int DDM_Mix_Solver_L2E::dc_solve()
{
  PetscInt rework = 1;
  PetscInt max_rework = 64;
  PetscInt Converged = 0;
...
/* ----------------------------------------------------------------------------
 * DDM_Mix_Solver_L2E::solution_update:  This function restore solution data from SNES
 */
void DDM_Mix_Solver_L2E::solution_update()
{
  //------------------------------------------------------------
  PetscScalar    *xx;
  VecGetArray(x,&xx);
...
/* ----------------------------------------------------------------------------
 * DDM_Mix_Solver_L2E::time_back_recovery:  This function recovery latest solution data
 * if NGSPICE time drops back.
 */
void DDM_Mix_Solver_L2E::time_back_recovery()
{
  //------------------------------------------------------------
  PetscScalar    *xx;
  VecGetArray(x,&xx);
  ...
  for(int z=0;z<zone_num;z++)
  {
    if(zonedata[z]->material_type==Semiconductor)
    {
      ...
      for(int i=0;i<zone[z].davcell.size();i++)
      {
        ...
        pzonedata->fs[i].p = pzonedata->fs[i].p_last;
        pzonedata->fs[i].T = pzonedata->fs[i].T_last;
        offset += 4;
      }
    }
    if(zonedata[z]->material_type==Insulator)
...
/* ----------------------------------------------------------------------------
 * DDM_Mix_Solver_L2E::diverged_recovery:  This function recovery latest solution data
 * if SNES diverged.
 */
void DDM_Mix_Solver_L2E::diverged_recovery()
{
  //------------------------------------------------------------
  PetscScalar    *xx;
  VecGetArray(x,&xx);
...
/* ----------------------------------------------------------------------------
 * DDM_Mix_Solver_L2E::destroy_solver:  This function do destroy the nonlinear solver
 */
int DDM_Mix_Solver_L2E::destroy_solver(SolveDefine &sv)
{
  // free work space
  N = 0;
  zofs.clear();

 ...
 */
void DDM_Solver_L2E::form_function_pn_2E(PetscScalar *x,PetscScalar *f)
{
  ...
  for(int z=0;z<zone_num;z++)
    if(zonedata[z]->material_type==Semiconductor)
    {
      ...
      for(int i=0;i<zone[z].datri.size();i++)
      {
        ...
        pzonedata->F2E_Tri_ddm(ptri,x,f,zofs);
      }
    }
  ...
  for(int z=0;z<zone_num;z++)
  {
    ...
    if(zonedata[z]->material_type==Semiconductor)
    {
      ...
      for(int i=0;i<pzonedata->electrode.size();i++)
      {
        if(bc[pzonedata->electrode[i]].BCType==OhmicContact)
          pzonedata->F2E_om_electrode(i,x,f,ODE_formula,zofs,bc,DeviceDepth);
        if(bc[pzonedata->electrode[i]].BCType==SchottkyContact)
          pzonedata->F2E_stk_electrode(i,x,f,ODE_formula,zofs,bc,DeviceDepth);
        if(bc[pzonedata->electrode[i]].BCType==InsulatorContact)
          pzonedata->F2E_ins_electrode(i,x,f,ODE_formula,zofs,bc,DeviceDepth);
      }
      ...
      for(int i=0;i<zone[z].davcell.size();i++)
      {
        ...
        if(!pcell->bc_index)
          pzonedata->F2E_ddm_inner(i,x,f,ODE_formula,zofs);
        else if(pcell->bc_index&&bc[pcell->bc_index-1].BCType==NeumannBoundary)
          pzonedata->F2E_ddm_neumannbc(i,x,f,ODE_formula,zofs,bc);
        else if(pcell->bc_index&&bc[pcell->bc_index-1].BCType==OhmicContact)
        {
          ...
          if(pbc->pinterface)//the ohmic bc is an electrode region
          {
            ...
            pzonedata->F2E_ddm_ombc_interface(i,x,f,ODE_formula,zofs,bc,pz,n_node);
          }
          else //the ohmic bc is a segment
            pzonedata->F2E_ddm_ombc_segment(i,x,f,ODE_formula,zofs,bc);
        }
        else if(pcell->bc_index&&bc[pcell->bc_index-1].BCType==SchottkyContact)
        {
          ...
          if(pbc->pinterface)//the Schottky bc is an electrode region
          {
            ...
            int n_node = pbc->pinterface->Find_neighbor_node_index(z,i);
            ElZone * pz = dynamic_cast<ElZone *>(zonedata[n_zone]);
            pzonedata->F2E_ddm_stkbc_interface(i,x,f,ODE_formula,zofs,bc,pz,n_node);
          }
          else //the Schottky bc is a segment
            pzonedata->F2E_ddm_stkbc_segment(i,x,f,ODE_formula,zofs,bc);
        }
        else if(pcell->bc_index&&bc[pcell->bc_index-1].BCType==InsulatorContact)
          pzonedata->F2E_ddm_insulator_gate(i,x,f,ODE_formula,zofs,bc);
        else if(pcell->bc_index&&bc[pcell->bc_index-1].BCType==InsulatorInterface)
        {
          ...
          pzonedata->F2E_ddm_interface(i,x,f,ODE_formula,zofs,bc,pz,n_node);
          //pzonedata->F2_ddm_interface(i,x,f,ODE_formula,zofs,bc,pz,n_node);
        }
        else if(pcell->bc_index&&bc[pcell->bc_index-1].BCType==HomoInterface)
        {
          ...
          pzonedata->F2E_ddm_homojunction(i,x,f,ODE_formula,zofs,bc,pz,n_node);
        }
        else if(pcell->bc_index&&bc[pcell->bc_index-1].BCType==HeteroInterface)
        {
          ...
          int n_node = pbc->pinterface->Find_neighbor_node_index(z,i);
          SMCZone * pz = dynamic_cast<SMCZone *>(zonedata[n_zone]);
          pzonedata->F2E_ddm_heterojunction(i,x,f,ODE_formula,zofs,bc,pz,n_node);
        }
    ...
    if(zonedata[z]->material_type==Insulator)
    {
      ...
      pzonedata->F2_efield_update(x,zofs,bc,zonedata);
      for(int i=0;i<zone[z].davcell.size();i++)
      {
        const VoronoiCell* pcell = zone[z].davcell.GetPointer(i);
        if(!pcell->bc_index)
          pzonedata->F2_ddm_inner(i,x,f,ODE_formula,zofs);
        else if(pcell->bc_index&&bc[pcell->bc_index-1].BCType==NeumannBoundary)
          pzonedata->F2_ddm_neumannbc(i,x,f,ODE_formula,zofs,bc);
        else if(pcell->bc_index && bc[pcell->bc_index-1].BCType==ChargedContact)
        {
          ...
          int n_node = pbc->pinterface->Find_neighbor_node_index(z,i);
          ElZone * pz = dynamic_cast<ElZone *>(zonedata[n_zone]);
          pzonedata->F2_ddm_chargebc_interface(i,x,f,ODE_formula,zofs,bc,pz,n_node);
        }
        else if(pcell->bc_index&&bc[pcell->bc_index-1].BCType==GateContact)
        {
          ...
          if(pbc->pinterface)//the gate bc is an electrode region
          {
            ...
            int n_node = pbc->pinterface->Find_neighbor_node_index(z,i);
            ElZone * pz = dynamic_cast<ElZone *>(zonedata[n_zone]);
            pzonedata->F2_ddm_gatebc_interface(i,x,f,ODE_formula,zofs,bc,pz,n_node);
          }
          else //the gate bc is a segment
            pzonedata->F2_ddm_gatebc_segment(i,x,f,ODE_formula,zofs,bc);
        }
        else if(pcell->bc_index && bc[pcell->bc_index-1].BCType==IF_Electrode_Insulator)
        {
          ...
          int n_node = pbc->pinterface->Find_neighbor_node_index(z,i);
          ElZone * pz = dynamic_cast<ElZone *>(zonedata[n_zone]);
          pzonedata->F2_ddm_electrode_insulator_interface(i,x,f,ODE_formula,zofs,bc,pz,n_node);
        }
        else if(pcell->bc_index&&bc[pcell->bc_index-1].BCType==InsulatorInterface)
        {
          InsulatorInterfaceBC *pbc;
          ...
          int n_node = pbc->pinterface->Find_neighbor_node_index(z,i);
          SMCZone * pz = dynamic_cast<SMCZone *>(zonedata[n_zone]);
          pzonedata->F2_ddm_semiconductor_insulator_interface(i,x,f,ODE_formula,zofs,bc,pz,n_node);
        }
        else if(pcell->bc_index && bc[pcell->bc_index-1].BCType==IF_Insulator_Insulator)
        {
          ...
          int n_node = pbc->pinterface->Find_neighbor_node_index(z,i);
          ISZone * pz = dynamic_cast<ISZone *>(zonedata[n_zone]);
          pzonedata->F2_ddm_insulator_insulator_interface(i,x,f,ODE_formula,zofs,bc,pz,n_node);
        }
        else  //prevent some mistakes caused by inexact bc
          pzonedata->F2_ddm_inner(i,x,f,ODE_formula,zofs);
      }
      for(int i=0;i<pzonedata->electrode.size();i++)
      {
        if(bc[pzonedata->electrode[i]].BCType==GateContact)
          pzonedata->F2_gate_electrode(i,x,f,ODE_formula,zofs,bc,DeviceDepth);
        if(bc[pzonedata->electrode[i]].BCType==ChargedContact)
          pzonedata->F2_charge_electrode(i,x,f,ODE_formula,zofs,bc);
      }
    }
    ...
    if(zonedata[z]->material_type==Conductor)
    {
      ...
      for(int i=0;i<zone[z].davcell.size();i++)
      {
        ...
        else if(pcell->bc_index && bc[pcell->bc_index-1].BCType==IF_Electrode_Insulator)
        {
          ...
          int n_node = pbc->pinterface->Find_neighbor_node_index(z,i);
          ISZone * pz = dynamic_cast<ISZone *>(zonedata[n_zone]);
          pzonedata->F2_ddm_elec_insulator_interface(i,x,f,ODE_formula,zofs,bc,pz,n_node);
        }
        else if(pcell->bc_index&&bc[pcell->bc_index-1].BCType==OhmicContact)
        {
          OhmicBC *pbc = dynamic_cast<OhmicBC*>(bc.Get_pointer(pcell->bc_index-1));
          ...
          int n_node = pbc->pinterface->Find_neighbor_node_index(z,i);
          SMCZone * pz = dynamic_cast<SMCZone *>(zonedata[n_zone]);
          pzonedata->F2_ddm_ombc_interface(i,x,f,ODE_formula,zofs,bc,pz,n_node);
        }
        else if(pcell->bc_index&&bc[pcell->bc_index-1].BCType==SchottkyContact)
        {
          SchottkyBC *pbc = dynamic_cast<SchottkyBC*>(bc.Get_pointer(pcell->bc_index-1));
          ...
          int n_node = pbc->pinterface->Find_neighbor_node_index(z,i);
          SMCZone * pz = dynamic_cast<SMCZone *>(zonedata[n_zone]);
          pzonedata->F2_ddm_stkbc_interface(i,x,f,ODE_formula,zofs,bc,pz,n_node);
        }
        else //prevent some mistakes caused by inexact bc
          pzonedata->F2_ddm_inner(i,x,f,ODE_formula,zofs);
      }
 ...
 */
void DDM_Solver_L2E::form_jacobian_pn_2E(PetscScalar *x, Mat *jac, Mat *jtmp)
{
  ...
  for(int z=0;z<zone_num;z++)
  {
    if(zonedata[z]->material_type==Semiconductor)
    {
      ...
      {
        Tri *ptri = &zone[z].datri[i];
        pzonedata->J2E_Tri_ddm(ptri,x,jtmp,zofs);
      }
    }
  }
  ...
  for(int z=0;z<zone_num;z++)
  {
    if(zonedata[z]->material_type==Semiconductor)
    {
      ...
      {
        if(bc[pzonedata->electrode[i]].BCType==OhmicContact)
          pzonedata->J2E_om_electrode(i,x,jac,jtmp,ODE_formula,zofs,bc,DeviceDepth);
        if(bc[pzonedata->electrode[i]].BCType==SchottkyContact)
          pzonedata->J2E_stk_electrode(i,x,jac,jtmp,ODE_formula,zofs,bc,DeviceDepth);
        if(bc[pzonedata->electrode[i]].BCType==InsulatorContact)
          pzonedata->J2E_ins_electrode(i,x,jac,jtmp,ODE_formula,zofs,bc,DeviceDepth);
      }
      ...
      for(int i=0;i<zone[z].davcell.size();i++)
      {
        ...
        if(!pcell->bc_index)
          pzonedata->J2E_ddm_inner(i,x,jac,jtmp,ODE_formula,zofs);
        else if(pcell->bc_index&&bc[pcell->bc_index-1].BCType==NeumannBoundary)
          pzonedata->J2E_ddm_neumannbc(i,x,jac,jtmp,ODE_formula,zofs,bc);
        else if(pcell->bc_index&&bc[pcell->bc_index-1].BCType==OhmicContact)
        {
          OhmicBC *pbc = dynamic_cast<OhmicBC*>(bc.Get_pointer(pcell->bc_index-1));
          if(pbc->pinterface)//the ohmic bc is an electrode region
          {
            ...
            int n_node = pbc->pinterface->Find_neighbor_node_index(z,i);
            ElZone * pz = dynamic_cast<ElZone *>(zonedata[n_zone]);
            pzonedata->J2E_ddm_ombc_interface(i,x,jac,jtmp,ODE_formula,zofs,bc,pz,n_node);
          }
          else //the ohmic bc is a segment
            pzonedata->J2E_ddm_ombc_segment(i,x,jac,jtmp,ODE_formula,zofs,bc);
        }
        else if(pcell->bc_index&&bc[pcell->bc_index-1].BCType==SchottkyContact)
        {
          SchottkyBC *pbc = dynamic_cast<SchottkyBC*>(bc.Get_pointer(pcell->bc_index-1));
          if(pbc->pinterface)//the Schottky bc is an electrode region
          {
            ...
            int n_node = pbc->pinterface->Find_neighbor_node_index(z,i);
            ElZone * pz = dynamic_cast<ElZone *>(zonedata[n_zone]);
            pzonedata->J2E_ddm_stkbc_interface(i,x,jac,jtmp,ODE_formula,zofs,bc,pz,n_node);
          }
          else //the Schottky bc is a segment
            pzonedata->J2E_ddm_stkbc_segment(i,x,jac,jtmp,ODE_formula,zofs,bc);
        }
        else if(pcell->bc_index&&bc[pcell->bc_index-1].BCType==InsulatorContact)
          pzonedata->J2E_ddm_insulator_gate(i,x,jac,jtmp,ODE_formula,zofs,bc);
        else if(pcell->bc_index&&bc[pcell->bc_index-1].BCType==InsulatorInterface)
        {
          InsulatorInterfaceBC *pbc;
          pbc = dynamic_cast<InsulatorInterfaceBC*>(bc.Get_pointer(pcell->bc_index-1));
          ...
          int n_node = pbc->pinterface->Find_neighbor_node_index(z,i);
          ISZone * pz = dynamic_cast<ISZone *>(zonedata[n_zone]);
          pzonedata->J2E_ddm_interface(i,x,jac,jtmp,ODE_formula,zofs,bc,pz,n_node);
          //pzonedata->J2_ddm_interface(i,x,jac,ODE_formula,zofs,bc,pz,n_node);
        }
        else if(pcell->bc_index&&bc[pcell->bc_index-1].BCType==HomoInterface)
        {
          HomoInterfaceBC *pbc;
          ...
          int n_node = pbc->pinterface->Find_neighbor_node_index(z,i);
          SMCZone * pz = dynamic_cast<SMCZone *>(zonedata[n_zone]);
          pzonedata->J2E_ddm_homojunction(i,x,jac,jtmp,ODE_formula,zofs,bc,pz,n_node);
        }
        else if(pcell->bc_index&&bc[pcell->bc_index-1].BCType==HeteroInterface)
        {
          HeteroInterfaceBC *pbc;
          ...
          int n_node = pbc->pinterface->Find_neighbor_node_index(z,i);
          SMCZone * pz = dynamic_cast<SMCZone *>(zonedata[n_zone]);
          pzonedata->J2E_ddm_heterojunction(i,x,jac,jtmp,ODE_formula,zofs,bc,pz,n_node);
        }
        else //prevent some mistakes caused by inexact bc
          pzonedata->J2E_ddm_inner(i,x,jac,jtmp,ODE_formula,zofs);
      }
    ...
    if(zonedata[z]->material_type==Insulator)
    {
      ...
      for(int i=0;i<zone[z].davcell.size();i++)
      {
        const VoronoiCell* pcell = zone[z].davcell.GetPointer(i);
        if(!pcell->bc_index)
          pzonedata->J2_ddm_inner(i,x,jac,ODE_formula,zofs);
        else if(pcell->bc_index&&bc[pcell->bc_index-1].BCType==NeumannBoundary)
          pzonedata->J2_ddm_neumannbc(i,x,jac,ODE_formula,zofs,bc);
        else if(pcell->bc_index && bc[pcell->bc_index-1].BCType==ChargedContact)
        {
          ChargedContactBC *pbc = dynamic_cast<ChargedContactBC*>(bc.Get_pointer(pcell->bc_index-1));
          int n_zone = pbc->pinterface->Find_neighbor_zone_index(z);
          int n_node = pbc->pinterface->Find_neighbor_node_index(z,i);
          ElZone * pz = dynamic_cast<ElZone *>(zonedata[n_zone]);
          pzonedata->J2_ddm_chargebc_interface(i,x,jac,ODE_formula,zofs,bc,pz,n_node);
        }
        else if(pcell->bc_index&&bc[pcell->bc_index-1].BCType==GateContact)
        {
          GateBC *pbc = dynamic_cast<GateBC*>(bc.Get_pointer(pcell->bc_index-1));
          if(pbc->pinterface)//the gate bc is an electrode region
          {
            ...
            int n_node = pbc->pinterface->Find_neighbor_node_index(z,i);
            ElZone * pz = dynamic_cast<ElZone *>(zonedata[n_zone]);
            pzonedata->J2_ddm_gatebc_interface(i,x,jac,ODE_formula,zofs,bc,pz,n_node);
          }
          else //the gate bc is a segment
            pzonedata->J2_ddm_gatebc_segment(i,x,jac,ODE_formula,zofs,bc);
        }
        else if(pcell->bc_index && bc[pcell->bc_index-1].BCType==IF_Electrode_Insulator)
        {
          ElectrodeInsulatorBC *pbc = dynamic_cast<ElectrodeInsulatorBC*>(bc.Get_pointer(pcell->bc_index-1));
          ...
          int n_node = pbc->pinterface->Find_neighbor_node_index(z,i);
          ElZone * pz = dynamic_cast<ElZone *>(zonedata[n_zone]);
          pzonedata->J2_ddm_electrode_insulator_interface(i,x,jac,ODE_formula,zofs,bc,pz,n_node);
        }
        else if(pcell->bc_index&&bc[pcell->bc_index-1].BCType==InsulatorInterface)
        {
          InsulatorInterfaceBC *pbc;
          ...
          int n_node = pbc->pinterface->Find_neighbor_node_index(z,i);
          SMCZone * pz = dynamic_cast<SMCZone *>(zonedata[n_zone]);
          pzonedata->J2_ddm_semiconductor_insulator_interface(i,x,jac,ODE_formula,zofs,bc,pz,n_node);
        }
        else if(pcell->bc_index && bc[pcell->bc_index-1].BCType==IF_Insulator_Insulator)
        {
          InsulatorInsulatorBC *pbc;
          ...
          int n_node = pbc->pinterface->Find_neighbor_node_index(z,i);
          ISZone * pz = dynamic_cast<ISZone *>(zonedata[n_zone]);
          pzonedata->J2_ddm_insulator_insulator_interface(i,x,jac,ODE_formula,zofs,bc,pz,n_node);
        }
        else  //prevent some mistakes caused by inexact bc
          pzonedata->J2_ddm_inner(i,x,jac,ODE_formula,zofs);
      }
      ...
      {
        if(bc[pzonedata->electrode[i]].BCType==GateContact)
          pzonedata->J2_gate_electrode(i,x,jac,ODE_formula,zofs,bc,DeviceDepth);
        if(bc[pzonedata->electrode[i]].BCType==ChargedContact)
          pzonedata->J2_charge_electrode(i,x,jac,ODE_formula,zofs,bc);
      }
    }
    ...
    if(zonedata[z]->material_type==Conductor)
    {
      ...
      for(int i=0;i<zone[z].davcell.size();i++)
      {
        ...
        else if(pcell->bc_index && bc[pcell->bc_index-1].BCType==IF_Electrode_Insulator)
        {
          ...
          int n_node = pbc->pinterface->Find_neighbor_node_index(z,i);
          ISZone * pz = dynamic_cast<ISZone *>(zonedata[n_zone]);
          pzonedata->J2_ddm_elec_insulator_interface(i,x,jac,ODE_formula,zofs,bc,pz,n_node);
        }
        else if(pcell->bc_index&&bc[pcell->bc_index-1].BCType==OhmicContact)
        {
          OhmicBC *pbc = dynamic_cast<OhmicBC*>(bc.Get_pointer(pcell->bc_index-1));
          ...
          int n_node = pbc->pinterface->Find_neighbor_node_index(z,i);
          SMCZone * pz = dynamic_cast<SMCZone *>(zonedata[n_zone]);
          pzonedata->J2_ddm_ombc_interface(i,x,jac,ODE_formula,zofs,bc,pz,n_node);
        }
        else if(pcell->bc_index&&bc[pcell->bc_index-1].BCType==SchottkyContact)
        {
          SchottkyBC *pbc = dynamic_cast<SchottkyBC*>(bc.Get_pointer(pcell->bc_index-1));
          ...
          int n_node = pbc->pinterface->Find_neighbor_node_index(z,i);
          SMCZone * pz = dynamic_cast<SMCZone *>(zonedata[n_zone]);
          pzonedata->J2_ddm_stkbc_interface(i,x,jac,ODE_formula,zofs,bc,pz,n_node);
        }
        else //prevent some mistakes caused by inexact bc
          pzonedata->J2_ddm_inner(i,x,jac,ODE_formula,zofs);
      }
...
/* ----------------------------------------------------------------------------
 * SNES_form_function_pn_2E:  wrap function for petsc nonlinear solver
 */
PetscErrorCode SNES_form_function_pn_2E(SNES snes, Vec x,Vec f,void *dummy)
{
  PetscScalar    *xx,*ff;
  DDM_Solver_L2E *ps = (DDM_Solver_L2E*)dummy;
  VecZeroEntries(f);
  //Get pointers to vector data.
  VecGetArray(x,&xx);
  VecGetArray(f,&ff);

  ps->form_function_pn_2E(xx,ff);
 ...
 */
PetscErrorCode SNES_form_jacobian_pn_2E(SNES snes, Vec x,Mat *jac,Mat *B,MatStructure *flag,void *dummy)
{
  PetscScalar    *xx;
  DDM_Solver_L2E *ps = (DDM_Solver_L2E*)dummy;
  ...
  //build matrix here
  ps->form_jacobian_pn_2E(xx,jac,&ps->JTmp);
...
/* ----------------------------------------------------------------------------
 * SNESMF_form_jacobian_pn_2E:  wrap function for petsc nonlinear solver with
 * matrix-free method
 */
PetscErrorCode SNESMF_form_jacobian_pn_2E(SNES snes, Vec x,Mat *A,Mat *B,MatStructure *flag,void *dummy)
{
  PetscScalar    *xx;
  DDM_Solver_L2E *ps = (DDM_Solver_L2E*)dummy;
  //Get pointer to vector data
  ...
  MatZeroEntries(ps->JTmp);
  //build matrix here
  ps->form_jacobian_pn_2E(xx,B,&ps->JTmp);
  *flag = SAME_NONZERO_PATTERN;
  //Restore vector
  VecRestoreArray(x,&xx);
  //Assemble matrix
...
/* ----------------------------------------------------------------------------
 * error_norm_pn_2E:  This function compute X and RHS error norms. 
 */
void DDM_Solver_L2E:: error_norm_estimat(PetscScalar *x,PetscScalar *f)
{
  // do clear
  potential_norm   = 0;
  electron_norm    = 0;
...
inline PetscErrorCode LineSearch_ConvergenceTest_L2E(SNES snes,PetscInt it,PetscReal xnorm, PetscReal pnorm, PetscReal fnorm,
    SNESConvergedReason *reason,void *dummy)
{
  ...
  *reason = SNES_CONVERGED_ITERATING;
  if (!it)
  {
    ...
    gss_log.string_buf()<<" "<<"its\t"<<"| Eq(V) |   "<<"| Eq(n) |   "<<"| Eq(p) |   "<<"| Eq(T) |   "<<"|delta x|\n"
    <<"----------------------------------------------------------------------\n";
    gss_log.record();
  }
...
inline PetscErrorCode TrustRegion_ConvergenceTest_L2E(SNES snes,PetscInt it,PetscReal xnorm, PetscReal pnorm, PetscReal fnorm,
    SNESConvergedReason *reason,void *dummy)
{
...
PetscErrorCode LimitorByBankRose_L2E(SNES snes, Vec x,Vec y,Vec w,void *checkctx, PetscTruth *changed_y,PetscTruth *changed_w)
{
  int it;
  DDM_Solver_L2E *ps = (DDM_Solver_L2E*)checkctx;
  PetscScalar    *xx;
  PetscScalar    *yy;
  PetscScalar    *ww;
  VecGetArray(x,&xx);
...
PetscErrorCode LimitorByPotential_L2E(SNES snes, Vec x,Vec y,Vec w,void *checkctx, PetscTruth *changed_y,PetscTruth *changed_w)
{
  PetscScalar    *xx;
  PetscScalar    *yy;
  PetscScalar    *ww;
...
PetscErrorCode LimitorNonNegativeCarrier_L2E(SNES snes, Vec x,Vec y,Vec w,void *checkctx, PetscTruth *changed_y,PetscTruth *changed_w)
{
  PetscScalar    *xx;
  PetscScalar    *yy;
  PetscScalar    *ww;
  ...
  for(int z=0;z<ps->zone_num;z++)
  {
    if(ps->zonedata[z]->material_type==Semiconductor)
    {
      ...
      { 
        PetscScalar r;
        if(fabs(yy[offset])>1.0)  {ww[offset] = xx[offset] - dsign(yy[offset])*1.0;flag=1;}
        if (ww[offset+1]<0.0) {ww[offset+1]=1.0*pow(ps->scale_unit.s_centimeter,-3);flag=1;}
        if (ww[offset+2]<0.0) {ww[offset+2]=1.0*pow(ps->scale_unit.s_centimeter,-3);flag=1;}
        if (ww[offset+3]<0.7*ps->LatticeTemp ) {ww[offset+3]=0.7*ps->LatticeTemp;flag=1;}
        offset += 4;        
      }
      offset += pzonedata->electrode.size();
...
/* ----------------------------------------------------------------------------
 * DDM_Solver_L2E::init_solver:  This function do initial setup for nonlinear solver
 */
int DDM_Solver_L2E::init_solver(SolveDefine &sv)
{
  gss_log.string_buf()<<"DDM solver Level 2E init...";
  //set Tolerances
  ...
  for(int i=0;i<zone_num;i++)
  {
    if(zonedata[i]->material_type==Semiconductor) //semiconductor zone
    {
      ...
      N+= 4*pzonedata->pzone->davcell.size();
  ...
  for(int z=0;z<zone_num;z++)
  {
    if(zonedata[z]->material_type==Semiconductor)
    {
      ...
      for(int i=0;i<zone[z].davcell.size();i++)
      {
        ...
        VecSetValues(x,4,index,init_value,INSERT_VALUES);
        offset += 4;
      }
  ...
  for(int i=0;i<zone_num;i++)
  {
    ...
    if(zonedata[i]->material_type==Semiconductor)
    {
      ...
      for(int j=0;j<pzonedata->pzone->davcell.size();j++)
      {
        ...
        {
          *p++ = 4*(pzonedata->pzone->davcell[j].nb_num+1);
          *p++ = 4*(pzonedata->pzone->davcell[j].nb_num+1);
          *p++ = 4*(pzonedata->pzone->davcell[j].nb_num+1);
          *p++ = 4*(pzonedata->pzone->davcell[j].nb_num+1);
        }
        ...
        {
          *p++ = 4*(2*pzonedata->pzone->davcell[j].nb_num+4);
          *p++ = 4*(2*pzonedata->pzone->davcell[j].nb_num+4);
          *p++ = 4*(2*pzonedata->pzone->davcell[j].nb_num+4);
          *p++ = 4*(2*pzonedata->pzone->davcell[j].nb_num+4);
        }
    ...
    //Insulator zones, poisson equation and heat transfer equation
  ...
  SNESSetFunction(snes,r,SNES_form_function_pn_2E,this);

  //if user defined a matrix-free method
  if(mf_flg)
  {
    gss_log.string_buf()<<"Matrix-Free...";
    SNESSetJacobian(snes,J,JPrec,SNESMF_form_jacobian_pn_2E,this);
  }
  else
  {
    SNESSetJacobian(snes,J,J,SNES_form_jacobian_pn_2E,this);
  }
  ...
  if(sv.NS==LineSearch || sv.NS==Basic)
  {
    ...
    if(sv.Damping==DampingBankRose)
      SNESLineSearchSetPostCheck(snes,LimitorByBankRose_L2E,this);
    else if(sv.Damping==DampingPotential)
       SNESLineSearchSetPostCheck(snes,LimitorByPotential_L2E,this);
    else
       SNESLineSearchSetPostCheck(snes,LimitorNonNegativeCarrier_L2E,this);
       
    SNESSetTolerances(snes,1e-12*N,1e-14,1e-9,maxit,1000);
    SNESSetConvergenceTest(snes,LineSearch_ConvergenceTest_L2E,this);
  }
  else if(sv.NS==TrustRegion)
  {
    ...
    SNESSetConvergenceTest(snes,TrustRegion_ConvergenceTest_L2E,this);
  ...
  else
  {
    ...
    KSPSetTolerances(ksp,1e-12*N,1e-20*N,PETSC_DEFAULT,max(35,N/10));
    PCSetType(pc,PCILU);
...
/* ----------------------------------------------------------------------------
 * DDM_Solver_L2E::do_solve:  This function solve the problem
 */
int DDM_Solver_L2E::do_solve(SolveDefine &sv)
{
  ...
  {
    gss_log.string_buf()<<"DDML2E not support this solver type!\n";
    gss_log.record();
  }
  return 0;
...
/* ----------------------------------------------------------------------------
 * DDM_Solver_L2E::solve_equ:  This function compute equilibrium
 * all the stimulate source(s) are zero. time step set to inf
 */
int DDM_Solver_L2E::solve_equ(SolveDefine &sv)
{
...
/* ----------------------------------------------------------------------------
 * DDM_Solver_L2E::solve_steadystate:  This function compute steadystate
 * set all the stimulate source(s) at there value of t=0. time step set to inf
 */
int DDM_Solver_L2E::solve_steadystate(SolveDefine &sv)
{
...
/* ----------------------------------------------------------------------------
 * DDM_Solver_L2E::solve_dcsweep:  This function compute IV curve
 * time step set to inf
 */
int DDM_Solver_L2E::solve_dcsweep(SolveDefine &sv)
{
  PetscScalar I=0;
  FILE *fiv;
...
/* ----------------------------------------------------------------------------
 * DDM_Solver_L2E::solve_iv_trace:  This function use continuation method to trace 
 * IV curve automatically
 */
int DDM_Solver_L2E::solve_iv_trace(SolveDefine &sv)
{
  int         error=0;
  int         first_step=1;
  int         slope_flag=0;
  ...
  while(pbc->Get_Potential()*sv.VStep < sv.VStop*sv.VStep && fabs(pbc->Get_Current())<sv.IStop)
  {
    ...
    do
    {
      ...
      if(recovery>8)
      {
        gss_log.string_buf()<<"Too many failure step, give up tring.\n\n\n";
        gss_log.record();
        error = 1;
        goto trace_end;
      }
    ...
    if(pbc->BCType==OhmicContact)
      pz->F2E_om_electrode_Trace(electrode_index,pdI_pdV,pdI_pdw,pdF_pdV,x,&JTmp,&J,ODE_formula,zofs,bc,DeviceDepth);
    if(pbc->BCType==SchottkyContact)
      pz->F2E_stk_electrode_Trace(electrode_index,pdI_pdV,pdI_pdw,pdF_pdV,x,&JTmp,&J,ODE_formula,zofs,bc,DeviceDepth);
...
void DDM_Solver_L2E::LET_norm_estimat(PetscScalar & r)
{
...
/* ----------------------------------------------------------------------------
 * DDM_Solver_L2E::solve_transient:  This function does transient simulation.
 */
int DDM_Solver_L2E::solve_transient(SolveDefine &sv)
{
...
/* ----------------------------------------------------------------------------
 * DDM_Solver_L2E::solver_pre_compute:  This function precompute some parameters before
 * each newton iteration.
 */
void DDM_Solver_L2E::solver_pre_compute()
{
  for(int z=0;z<zone_num;z++)
  {
    if(zonedata[z]->material_type==Semiconductor)
...
/* ----------------------------------------------------------------------------
 * DDM_Solver_L2E::solution_update:  This function restore solution data from SNES
 */
void DDM_Solver_L2E::solution_update()
{
  //------------------------------------------------------------
  PetscScalar    *xx;
  VecGetArray(x,&xx);
  ...
  for(int z=0;z<zone_num;z++)
  {
    if(zonedata[z]->material_type==Semiconductor)
    {
      ...
      pzonedata->F2E_efield_update(xx,zofs,bc,zonedata);
      for(int i=0;i<pzonedata->electrode.size();i++)
      {
    ...
    {
      ISZone *pzonedata = dynamic_cast< ISZone * >(zonedata[z]);
      pzonedata->F2_efield_update(xx,zofs,bc,zonedata);
      for(int i=0;i<zone[z].davcell.size();i++)
      {
        pzonedata->fs[i].P_last = pzonedata->fs[i].P;
        pzonedata->fs[i].T_last = pzonedata->fs[i].T;
...
/* ----------------------------------------------------------------------------
 * DDM_Solver_L2E::diverged_recovery:  This function recovery latest solution data
 * if SNES diverged.
 */
void DDM_Solver_L2E::diverged_recovery()
{
  //------------------------------------------------------------
  PetscScalar    *xx;
  VecGetArray(x,&xx);
...
/* ----------------------------------------------------------------------------
 * DDM_Solver_L2E::destroy_solver:  This function do destroy the nonlinear solver
 */
int DDM_Solver_L2E::destroy_solver(SolveDefine &sv)
{
  // free work space
  N = 0;
  zofs.clear();

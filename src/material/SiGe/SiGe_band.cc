...
class GSS_SiGe_BandStructure : public PMIS_BandStructure
{
private:
  PetscScalar T300;
  PetscScalar T90;
  ...
  PetscScalar EGSi300;   // The energy bandgap of the material at 300 K.
  PetscScalar EgSi90;
  ...
  PetscScalar NC_0;         // Parameters for effective density of states of compound semiconductor material
  PetscScalar NV_0;
  PetscScalar NC_E;
  PetscScalar NV_E;
  ...
  void Eg_Init()
  {
    EGSi300   = 1.080000e+00*eV;
    EGALPH    = 4.730000e-04*eV/K;
    EGBETA    = 6.360000e+02*K;
    T90       = 90*K;
    EgSi90    = EGSi300+EGALPH*(T300*T300/(T300+EGBETA) - T90*T90/(T90+EGBETA));
...
public:
  ...
  PetscScalar Eg (const PetscScalar &Tl)
  {
    ...
    PetscScalar EG90;
    if(mole_x<=0.25)
        EG90 = EgSi90 - 4.0*(EgSi90-0.950)*mole_x;
    else if(mole_x<=0.40)
        EG90 = 0.950 -  0.66666*(mole_x-0.25);
    else if(mole_x<=0.60)
        EG90 = 0.850 - 0.57500*(mole_x-0.40);
    else if(mole_x<=0.75)
        EG90 = 0.735-0.43333*(mole_x-0.60);
    else
        EG90 = 0.670;
    return EG90+EGALPH*(T90*T90/(T90+EGBETA) - Tl*Tl/(Tl+EGBETA));
  }
  ...
  {
    PetscScalar mole_x = ReadxMoleFraction();
    return NC300*pow(Tl/T300,NC_F)*(1+NC_0*(exp(-NC_E*mole_x/(kb*Tl))-1));
  }
  ...
  {
    PetscScalar mole_x = ReadxMoleFraction();
    return NV300*pow(Tl/T300,NV_F)*(1+NV_0*(exp(-NV_E*mole_x/(kb*Tl))-1));
  }
...
public:
  GSS_SiGe_BandStructure(const PMIS_Environment &env):PMIS_BandStructure(env)
  {
    T300 = 300.0*K;
    Eg_Init();
    Lifetime_Init();
...
extern "C"
{
  PMIS_BandStructure*  PMIS_SiGe_BandStructure_Default (const PMIS_Environment& env)
  {
    return new GSS_SiGe_BandStructure(env);
  }
}

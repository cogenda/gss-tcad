...
class GSS_InN_BandStructure : public PMIS_BandStructure
{
private:
  PetscScalar T300;
  //[Bandgap]
  ...
  void Eg_Init()
  {
    ...
    NC300     = 1.3000e+18*pow(cm,-3);
    NV300     = 5.3000e+19*pow(cm,-3);
    NC_F      = 1.500000e+00;
    NV_F      = 1.500000e+00;
...
private:
  ...
  void Recomb_Init()
  {
    ...
    AUGN    =  1.0000e-30*pow(cm,6)/s;
    AUGP    =  1.0000e-30*pow(cm,6)/s;
...
public:
  ...
  {
    return 0.3e-12*s;//Source: data base of DESSIS
  }
...
public:
  GSS_InN_BandStructure(const PMIS_Environment &env):PMIS_BandStructure(env)
  {
    T300 = 300.0*K;
    Eg_Init();
    Lifetime_Init();
...
extern "C"
{
  PMIS_BandStructure*  PMIS_InN_BandStructure_Default (const PMIS_Environment& env)
  {
    return new GSS_InN_BandStructure(env);
  }
}

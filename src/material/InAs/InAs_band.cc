...
class GSS_InAs_BandStructure : public PMIS_BandStructure
{
private:
  PetscScalar T300;
  //[Bandgap]
  ...
  void Eg_Init()
  {
    ...
    EG300     = 0.4105*eV;
    EGALPH    = 3.3500e-04*eV/K;
    EGBETA    = 2.4800e+02*K;

    NC300     = 8.7200e+16*pow(cm,-3);
    NV300     = 6.6600e+18*pow(cm,-3);
    NC_F      = 1.500000e+00;
    NV_F      = 1.500000e+00;
...
private:
  ...
  void Recomb_Init()
  {
    ...
    AUGN    =  2.200000e-27*pow(cm,6)/s;
    AUGP    =  2.200000e-27*pow(cm,6)/s;
    C_DIRECT = 1.100000e-10*pow(cm,3)/s;
  }
...
public:
  GSS_InAs_BandStructure(const PMIS_Environment &env):PMIS_BandStructure(env)
  {
    T300 = 300.0*K;
    Eg_Init();
    Lifetime_Init();
...
extern "C"
{
  PMIS_BandStructure*  PMIS_InAs_BandStructure_Default (const PMIS_Environment& env)
  {
    return new GSS_InAs_BandStructure(env);
  }
}

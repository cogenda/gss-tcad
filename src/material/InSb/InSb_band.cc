...
class GSS_InSb_BandStructure : public PMIS_BandStructure
{
private:
  PetscScalar T300;
  //[Bandgap]
  ...
  void Eg_Init()
  {
    //Source: ATLAS user's guide
    EG300     = 0.235*eV;
    EGALPH    = 0.32e-3*eV/K;
    EGBETA    = 170.0*K;

    NC300     = 4.16000e+16*pow(cm,-3);
    NV300     = 8.35000e+18*pow(cm,-3);
    NC_F      = 1.500000e+00;
    NV_F      = 1.500000e+00;
...
private:
  ...
  void Lifetime_Init()
  {
    ...
    TAUN0     = 5.000000e-08*s;
    TAUP0     = 5.000000e-08*s;
...
private:
  ...
  void Recomb_Init()
  {
    ...
    AUGN    =  5.0000e-26*pow(cm,6)/s;
    AUGP    =  5.0000e-26*pow(cm,6)/s;
    C_DIRECT = 5.000000e-11*pow(cm,3)/s;
  }
...
public:
  ...
  {
    return 1.0e-12*s;//This value is not exact
  }
  ...
  {
    return 0.5e-12*s;//This value is not exact
  }
...
public:
  GSS_InSb_BandStructure(const PMIS_Environment &env):PMIS_BandStructure(env)
  {
    T300 = 300.0*K;
    Eg_Init();
    Lifetime_Init();
...
extern "C"
{
  PMIS_BandStructure*  PMIS_InSb_BandStructure_Default (const PMIS_Environment& env)
  {
    return new GSS_InSb_BandStructure(env);
  }
}

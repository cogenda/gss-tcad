...
class DDM_Mix_Solver_L2E : virtual public BSolver
{
private:
...
public:
  ...
  DDM_Mix_Solver_L2E():N(0),its(0),norm(0.0) {} ;
  ~DDM_Mix_Solver_L2E() {};
  void form_function_pn_Mix2(PetscScalar *x,PetscScalar *f);
  void form_jacobian_pn_Mix2(PetscScalar *x, Mat *jac, Mat *jtmp);
  void error_norm_pn_Mix2(PetscScalar *x,PetscScalar *f);
  int  DEV_LOAD();
  int  DEV_ACCEPT();
  int  DEV_CONV_TEST();
  int  tran_solve();
